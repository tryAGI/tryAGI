...
| `--generate-polymorphic-array-helpers` | Runtime `AutoSDKPolymorphicFormat` base class + `[AutoSDKPolymorphicFormatVariant("name", typeof(...))]` attribute + `AutoSDKPolymorphicFormatJsonConverter<TBase>` that round-trips array items shaped as `oneOf{string-enum, object-with-type-const}` (Firecrawl v2 scrape `formats`, OpenAI Responses `tools`, Anthropic tool-use `content`). The generator now also **auto-detects** the pattern and emits typed `<Parent><Property>Item` base classes plus sealed bare-string variants (`MarkdownFormat`, `HtmlFormat`, ...) and typed object variants (`JsonFormat`, `HighlightsFormat`, ...) with the converter already wired. Variant property types beyond scalars + scalar arrays currently fall back to `object?` for the consumer to refine in a partial class. | Firecrawl v2, OpenAI Responses, Anthropic tool-use, any spec with `oneOf{string, object}` array items |
