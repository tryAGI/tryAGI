...
| `--generate-polymorphic-array-helpers` | Runtime `AutoSDKPolymorphicFormat` base class + `[AutoSDKPolymorphicFormatVariant("name", typeof(...))]` attribute + `AutoSDKPolymorphicFormatJsonConverter<TBase>` that round-trips array items shaped as `oneOf{string-enum, object-with-type-const}` (Firecrawl v2 scrape `formats`, OpenAI Responses `tools`, Anthropic tool-use `content`). Consumer code hand-models the variant subclasses; the converter handles bare-string ↔ typed-object dispatch via the `type` discriminator. Full automatic detection in the generator is deferred. | Firecrawl v2, OpenAI Responses, Anthropic tool-use, any spec with `oneOf{string, object}` array items |
